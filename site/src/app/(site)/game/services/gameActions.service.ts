...
import { GAME_CONFIG, NGU_HANH_RELATIONS, GAME_PHASES } from '../constants/game.constants';
...
export class GameActionsService {
  ...
  static harvestSource(gameState: GameState, sourceKey: string, isOffline = false): ActionResult {
    const { currentMapId, maps, resources } = gameState;
    const currentMapData = maps[currentMapId];
    if (!currentMapData) {
      return { success: false, message: 'No map data available.' };
  ...
  private static getDiscoveryCostType(hiddenType: string): keyof GameResources {
    ...
    const elementType =
      NGU_HANH_RELATIONS.elementMap[hiddenType as keyof typeof NGU_HANH_RELATIONS.elementMap];
    const generatesType =
      NGU_HANH_RELATIONS.generates[elementType as keyof typeof NGU_HANH_RELATIONS.generates];
  ...
  private static calculateHarvestYield(source: any, spiritBeasts: any) {
    const primaryResourceType =
      NGU_HANH_RELATIONS.elementMap[source.type as keyof typeof NGU_HANH_RELATIONS.elementMap];
