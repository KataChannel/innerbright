...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ error: 'Không thể tải thông tin phòng ban' }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (department.employees.length > 0) {
      return NextResponse.json({ error: 'Không thể xóa phòng ban còn nhân viên' }, { status: 400 });
    }

    // Kiểm tra có chức vụ hay không
    if (department.positions.length > 0) {
      return NextResponse.json({ error: 'Không thể xóa phòng ban còn chức vụ' }, { status: 400 });
    }
