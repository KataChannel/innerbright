#!/usr/bin/env bun
/**
 * Innerbright Master Seed Script
...
async function seedSystemUsers(roles: any) {
  ...
  const admin = await prisma.user.create({
    data: {
      ...
      bio: 'Quản trị viên hệ thống Innerbright',
      isVerified: true,
      isActive: true,
      status: UserStatus.ONLINE,
...
async function seedDepartments(users: any) {
  ...
  const ceoOffice = await prisma.department.create({
    data: {
      ...
      code: 'CEO',
      budget: 2000000000,
      location: 'Tầng 10, Tòa nhà Innerbright',
      phone: '+84123456789',
      email: 'ceo@innerbright.com',
  ...
  const hrDepartment = await prisma.department.create({
    data: {
      ...
      code: 'HR',
      budget: 800000000,
      location: 'Tầng 2, Tòa nhà Innerbright',
      phone: '+84123456790',
  ...
  const itDepartment = await prisma.department.create({
    data: {
      ...
      code: 'IT',
      budget: 1500000000,
      location: 'Tầng 3-4, Tòa nhà Innerbright',
      phone: '+84123456791',
  ...
  const salesDepartment = await prisma.department.create({
    data: {
      ...
      code: 'SALES',
      budget: 1200000000,
      location: 'Tầng 1, Tòa nhà Innerbright',
      phone: '+84123456792',
  ...
  const financeDepartment = await prisma.department.create({
    data: {
      ...
      code: 'FIN',
      budget: 600000000,
      location: 'Tầng 5, Tòa nhà Innerbright',
      phone: '+84123456793',
  ...
  const operationsDepartment = await prisma.department.create({
    data: {
      ...
      code: 'OPS',
      budget: 700000000,
      location: 'Tầng 6, Tòa nhà Innerbright',
      phone: '+84123456794',
      email: 'operations@innerbright.com',
      parentId: ceoOffice.id,
      isActive: true,
    }
  });
...
async function seedHRAndDepartmentUsers(roles: any, departments: any, positions: any) {
  ...
  const financeManager = await prisma.user.create({
    data: {
      email: 'finance.manager@innerbright.com',
  ...
  const operationsManager = await prisma.user.create({
    data: {
      email: 'ops.manager@innerbright.com',
...
async function seedEmployees(roles: any, departments: any, positions: any, managers: any, systemUsers: any) {
  ...
  const itEmployees = await Promise.all([
    prisma.user.create({
      data: {
        email: 'john.doe@innerbright.com',
        username: 'john_doe',
        phone: '+84902000001',
    ...
    prisma.user.create({
      data: {
        email: 'jane.smith@innerbright.com',
        username: 'jane_smith',
        phone: '+84902000002',
    ...
    prisma.user.create({
      data: {
        email: 'david.wilson@innerbright.com',
  ...
  const salesEmployees = await Promise.all([
    prisma.user.create({
      data: {
        email: 'sarah.johnson@innerbright.com',
    ...
    prisma.user.create({
      data: {
        email: 'mike.brown@innerbright.com',
        username: 'mike_brown',
        phone: '+84902000005',
  ...
  const hrEmployees = await Promise.all([
    prisma.user.create({
      data: {
        email: 'lisa.garcia@innerbright.com',
  ...
  const financeEmployees = await Promise.all([
    prisma.user.create({
      data: {
        email: 'robert.davis@innerbright.com',
...
async function createEmployeeRecords(users: any, departments: any, positions: any) {
  ...
  const ceoEmployee = await prisma.employee.create({
    data: {
      ...
      notes: 'Founder & CEO của Innerbright',
...
async function seedChatData(users: any, roles: any) {
  ...
  const messages = await Promise.all([
    prisma.message.create({
      data: {
        content: 'Chào mừng tất cả mọi người đến với Innerbright! 🎉',
 ...
 */
async function main() {
  console.log(`${colors.bright}${colors.cyan}🌱 Innerbright Master Seed Starting...${colors.reset}\n`);
  ...
  try {
    ...
    console.log(`   Super Admin: superadmin@innerbright.com / SuperAdmin@2024`);
    console.log(`   Admin: admin@innerbright.com / Admin@2024`);
    console.log(`   HR Manager: hr.manager@innerbright.com / Hr@2024`);
    console.log(`   IT Director: it.director@innerbright.com / It@2024`);
    console.log(`   Sales Manager: sales.manager@innerbright.com / Sales@2024`);
