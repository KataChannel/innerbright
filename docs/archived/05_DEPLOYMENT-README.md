...
Complete guide for deploying innerbright using the automated deployment scripts.
...
./deploy-production.sh 116.118.85.41 yourdomain.com
```
...
./deploy-remote.sh --key ~/.ssh/my-custom-key.pem 116.118.85.41 domain.com
```
...
./deploy-remote.sh --force-regen 116.118.85.41 myapp.com
```
...
ssh -i ~/.ssh/default root@116.118.85.41
...
docker compose exec postgres psql -U innerbright -d innerbright
```
...
**Built with ❤️ by the innerbright Team**
