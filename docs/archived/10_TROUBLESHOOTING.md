# 🔧 innerbright Troubleshooting Guide
...
1. **Check Database Performance**
   ...
   # Check database size
   SELECT pg_database_size('innerbright');
   ```
...
1. **Check Database Status**
   ...
   # Test connection
   docker-compose exec postgres psql -U postgres -d innerbright -c "SELECT 1;"
   ```
...
2. **Backup Database Before Migration**
   ```bash
   # Create backup
   docker-compose exec postgres pg_dump -U postgres innerbright > backup.sql
...
2. **Check Docker Network**
   ...
   docker network inspect innerbright_default
...
docker-compose exec postgres psql -U postgres -d innerbright -c "SELECT 1;" > /dev/null 2>&1
if [ $? -eq 0 ]; then
    echo "Database: OK"
else
    echo "Database: ERROR"
...
echo "=== innerbright Performance Monitor ==="
...
docker-compose exec postgres psql -U postgres -d innerbright -c "
SELECT 
    calls,
    mean_time,
    query
...
2. **Contact Support:**
   - Email: emergency@innerbright.com
...
**Community Resources:**
- Discord: https://discord.gg/innerbright
- GitHub Issues: https://github.com/innerbright/issues
- Stack Overflow: Tag `innerbright`
