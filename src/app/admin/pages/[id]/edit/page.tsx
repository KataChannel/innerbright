...
export default function EditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchPage = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setIsHomePage(pageData.isHomePage);
  ...
  const handleSave = async (saveStatus: 'DRAFT' | 'PUBLISHED' | 'ARCHIVED' = status) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(error.error || 'Có lỗi xảy ra khi cập nhật trang');
      }
    } catch (error) {
      console.error('Error updating page:', error);
      alert('Có lỗi xảy ra khi cập nhật trang');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async () => {
    if (!confirm('Bạn có chắc chắn muốn xóa trang này? Thao tác này không thể hoàn tác.')) {
      return;
    }
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-2xl font-bold text-gray-900">Chỉnh sửa trang: {page.title}</h1>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  <span className="text-sm text-gray-600">Tạo:</span>
                ...
                {page.publishedAt && (
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm font-medium">
                      {new Date(page.publishedAt).toLocaleDateString('vi-VN')}
                    </span>
                  </div>
                )}
              </div>
